...
# Implement caching for the Google Sheets connection
@st.cache_resource
def init_connection():
    """
    Initializes Google Sheets connection using the service account credentials.
    Caches the connection to minimize API calls.
    """
    scope = [
    ...
    try:
        ...
        st.error(f"Error initializing Google Sheets connection: {e}")
        st.stop()

# Implement caching for fetching worksheets
...
def get_worksheet(client, sheet_name, worksheet_name):
    ...
    try:
        ...
        st.error(f"The spreadsheet '{sheet_name}' was not found.")
        st.stop()
    except APIError as e:
        st.error(f"API Error while opening spreadsheet '{sheet_name}': {e}")
        st.stop()
    except Exception as e:
        st.error(f"Unexpected error while opening spreadsheet '{sheet_name}': {e}")
        st.stop()
    ...
    try:
        ...
        st.error(f"The worksheet '{worksheet_name}' was not found in '{sheet_name}'.")
        st.stop()
    except APIError as e:
        st.error(f"API Error while accessing worksheet '{worksheet_name}': {e}")
        st.stop()
    except Exception as e:
        st.error(f"Unexpected error while accessing worksheet '{worksheet_name}': {e}")
        st.stop()

# Implement caching for fetching data frames
@st.cache_data(ttl=300)
def get_dataframe(worksheet):
    """
    Fetches all records from a worksheet and returns a pandas DataFrame.
    Caches the DataFrame to minimize API calls.
    """
    try:
        ...
        st.error(f"API Error while fetching data: {e}")
        st.stop()
    except Exception as e:
...
def find_account_by_id(accounts_ws, user_id):
    ...
    try:
        ...
        st.error(f"API Error while searching for ID {user_id}: {e}")
    except Exception as e:
        st.error(f"Unexpected error while searching for ID {user_id}: {e}")
    return None

def create_account(accounts_ws, user_id, name, company, creator_agent, branch, can_negative_balance, phone_number, registered_by):
    """
    Creates a new account in the 'accounts' worksheet.
    Returns True if successful, or False if the account already exists.
    """
    ...
    try:
        ...
        st.error(f"API Error while creating account: {e}")
    except Exception as e:
        st.error(f"Unexpected error while creating account: {e}")
    return False

def record_transaction_with_retry(transactions_ws, user_id, transaction_type, amount, branch, agent_name, retries=5, backoff_factor=2):
    """
    Attempts to record a transaction with retry logic in case of API rate limiting.
    """
    for attempt in range(retries):
        try:
            ...
            transactions_ws.append_row(row_data, value_input_option="USER_ENTERED")
            st.success(f"Transaction recorded: {'+' if transaction_type == 'ADD' else '-'}{amount} {'to' if transaction_type == 'ADD' else 'from'} ID {user_id}.")
            return
        except APIError as e:
            if e.response.status_code == 429:
                sleep_time = backoff_factor ** attempt
                st.warning(f"Rate limit exceeded. Retrying in {sleep_time} seconds...")
                time.sleep(sleep_time)
            else:
                st.error(f"API Error while recording transaction: {e}")
                return
        except Exception as e:
            st.error(f"Unexpected error while recording transaction: {e}")
            return
    st.error("Failed to record transaction after multiple attempts due to rate limiting.")
...
def get_account_data(accounts_ws, row_num):
    ...
    try:
        ...
        st.error(f"API Error while fetching account data: {e}")
    except Exception as e:
        st.error(f"Unexpected error while fetching account data: {e}")
    return {}

def update_account_data(accounts_ws, row_num, name, company, creator_agent, can_negative_balance, phone_number, registered_by, branch):
    """
    Updates the editable fields in the 'accounts' worksheet.
    Does NOT change ID (col A) or Timestamp (col E).
    """
    try:
        # Update Name (col 2)
        accounts_ws.update_cell(row_num, 2, name)
        ...
        st.error(f"API Error while updating account: {e}")
    except Exception as e:
        st.error(f"Unexpected error while updating account: {e}")
...
def get_transactions_for_id(transactions_ws, user_id):
    ...
    try:
        ...
        user_transactions = [record for record in records if str(record.get('ID', '')).strip() == str(user_id)]
        return user_transactions
    except APIError as e:
        st.error(f"API Error while fetching transactions for ID {user_id}: {e}")
    except Exception as e:
        st.error(f"Unexpected error while fetching transactions for ID {user_id}: {e}")
    return []
...
def verify_user(users_ws, username, password):
    ...
    try:
        ...
        st.error(f"API Error while verifying user: {e}")
    except Exception as e:
        st.error(f"Unexpected error while verifying user: {e}")
    return False

def get_user_info(users_ws, username):
    """
    Fetch user data from 'users' sheet, specifically the 'negative_access' and 'edit_access' columns.
    Assumes 'users' sheet columns:
       A: username
       B: password
       C: negative_access
       D: edit_access
    ...
    try:
        ...
        negative_access = negative_access.strip().lower() if negative_access else "false"
        edit_access = edit_access.strip().lower() if edit_access else "false"
        ...
        st.error(f"API Error while fetching user info: {e}")
    except Exception as e:
        st.error(f"Unexpected error while fetching user info: {e}")
    return {"negative_access": "false", "edit_access": "false"}

def get_user_balance(user_balances_ws, user_id):
    """
    Reads the user's current balance from the 'user_balances' worksheet.
    Expects columns named 'id' and 'balance'.
    Returns the balance as a float or 0.0 if not found or not parseable.
    """
    try:
        records = user_balances_ws.get_all_records()
        for rec in records:
            if str(rec.get("id", "")).strip() == str(user_id):
                try:
                    return float(rec.get("balance", 0.0))
                except ValueError:
                    return 0.0
    except APIError as e:
        st.error(f"API Error while fetching balance for ID {user_id}: {e}")
    except Exception as e:
        st.error(f"Unexpected error while fetching balance for ID {user_id}: {e}")
    return 0.0
...
def fetch_all_ids(accounts_ws):
    ...
    try:
        ...
        return [id.strip() for id in id_column if id.strip() and id.lower() != "id"]
    except APIError as e:
        st.error(f"API Error while fetching IDs: {e}")
    except Exception as e:
        st.error(f"Unexpected error while fetching IDs: {e}")
    return []
    ...
    if st.button("Search"):
        ...
        if not user_id:
            ...
            if row_num is None:
                ...
                account_data = get_account_data(accounts_ws, row_num)
                if not account_data:
                    st.error(f"Failed to retrieve data for ID {user_id}.")
                    return
                st.session_state.edit_data = {
                    "row_num": row_num,
    ...
    if "edit_data" in st.session_state and "account_data" in st.session_state.edit_data:
        ...
        with st.form("edit_account_form"):
            ...
            if submitted_edit:
                ...
                # Optionally, remove edit_data to prevent re-editing
                del st.session_state["edit_data"]
...
def page_transaction(accounts_ws, transactions_ws, user_balances_ws):
    ...
    with st.form("transaction_form"):
        ...
        if submitted:
            ...
            # Get account data and current balance
            account_data = get_account_data(accounts_ws, row_num)
            current_balance = get_user_balance(user_balances_ws, user_id)
            ...
            record_transaction_with_retry(transactions_ws, user_id, transaction_type, amount, branch, agent_name)
...
def page_search(accounts_ws, transactions_ws, user_balances_ws):
    ...
    if st.button("Search"):
        ...
        # Highlight function for negative vs positive balance using Styler.map
        def highlight_balance(val):
            if "EGP" in val:
                numeric_part = val.replace(' EGP', '').replace(',', '')
                try:
                    numeric_val = float(numeric_part)
                    if numeric_val < 0:
                        color = 'red'
...
def page_audit_dashboard(accounts_ws, transactions_ws, user_balances_ws):
    ...
    # --- Fetch raw data from Google Sheets using cached functions
    df_accounts = get_dataframe(accounts_ws)
    df_transactions = get_dataframe(transactions_ws)
    df_balances = get_dataframe(user_balances_ws)
    ...
    # B) Branch filter (with "All" option)
    branch_options = sorted(df_transactions['Branch'].dropna().unique()) if 'Branch' in df_transactions.columns else []
    branch_options = ["All"] + branch_options
    selected_branch_t = st.selectbox("Transaction Branch", branch_options, index=0)
    ...
    #    Note: Transactions themselves don't store company, but we can join on the "ID"
    #    to get the user’s company from df_accounts.
    company_options = sorted(df_accounts['Company'].dropna().unique()) if 'Company' in df_accounts.columns else []
    company_options = ["All"] + company_options
    selected_company_t = st.selectbox("Transaction Company", company_options, index=0)
    ...
    if not df_transactions_filtered.empty and 'Timestamp' in df_transactions_filtered.columns:
        df_transactions_filtered = df_transactions_filtered[
            (df_transactions_filtered['Timestamp'].dt.date >= start_date_t) &
            (df_transactions_filtered['Timestamp'].dt.date <= end_date_t)
        ]
    ...
    if selected_company_t != "All":
        if not df_accounts.empty and 'ID' in df_accounts.columns and 'Company' in df_accounts.columns:
            # Merge to bring "Company" into the transactions
            merged_t = df_transactions_filtered.merge(df_accounts[['ID','Company']], 
                                                      left_on='ID', 
                                                      right_on='ID', 
                                                      how='left')
    ...
    # B) User Company (with "All" option)
    company_options_u = sorted(df_accounts['Company'].dropna().unique()) if 'Company' in df_accounts.columns else []
    company_options_u = ["All"] + company_options_u
    selected_company_u = st.selectbox("User Company", company_options_u, index=0)

    # C) User Branch (with "All" option)
    branch_options_u = sorted(df_accounts['Branch'].dropna().unique()) if 'Branch' in df_accounts.columns else []
    branch_options_u = ["All"] + branch_options_u
    selected_branch_u = st.selectbox("User Branch", branch_options_u, index=0)
    ...
    if not df_accounts_filtered.empty and 'Timestamp' in df_accounts_filtered.columns:
        df_accounts_filtered = df_accounts_filtered[
            (df_accounts_filtered['Timestamp'].dt.date >= start_date_u) &
            (df_accounts_filtered['Timestamp'].dt.date <= end_date_u)
        ]
    ...
    # Merge with balances to get CurrentBalance
    if not df_balances.empty and 'id' in df_balances.columns and 'balance' in df_balances.columns:
        df_balances.rename(columns={'id': 'ID'}, inplace=True)
        df_balances['ID'] = df_balances['ID'].astype(str).str.strip()
        df_balances['balance'] = pd.to_numeric(df_balances['balance'], errors='coerce').fillna(0.0)
        df_accounts_filtered = df_accounts_filtered.merge(df_balances[['ID', 'balance']], on='ID', how='left')
        df_accounts_filtered['balance'] = df_accounts_filtered['balance'].fillna(0.0)
    else:
        df_accounts_filtered['balance'] = 0.0  # Default if balances data missing

    # Apply balance tag filter
    if selected_balance_tag != "All":
        if selected_balance_tag == "no_balance":
            df_accounts_filtered = df_accounts_filtered[df_accounts_filtered['balance'] == 0.0]
        elif selected_balance_tag == "positive_balance":
            df_accounts_filtered = df_accounts_filtered[df_accounts_filtered['balance'] > 0.0]
        elif selected_balance_tag == "negative_balance":
            df_accounts_filtered = df_accounts_filtered[df_accounts_filtered['balance'] < 0.0]
    ...
    if df_accounts_filtered.empty:
        ...
        df_display = df_accounts_filtered.copy()
        df_display['CurrentBalance'] = df_display['balance'].apply(lambda x: f"{x:,.2f} EGP")
        df_display.drop(columns=['balance'], inplace=True)

        # Highlight balances using Styler.map
        def highlight_balance(val):
            try:
                num = float(val.replace(',', '').replace(' EGP', ''))
                if num > 0:
                    color = 'green'
                elif num < 0:
                    color = 'red'
...
def main():
    ...
    # --------------------------------------------
    with st.sidebar:
        # Conditionally add "Audit Dashboard" based on user access
        if st.session_state.get("edit_access", "false") == "true" or st.session_state.get("negative_access", "false") == "true":
            page_list = [
                "Create Account", 
                "Transaction Recorder", 
                "Search Account", 
